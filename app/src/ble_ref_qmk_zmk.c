...
extern uint8_t get_identity_by_profile_config(void);
...
enum advertising_type get_adv_status(void) { return advertising_status; };
...
bool zmk_ble_profile_is_connected(uint8_t index) {
    ...
    uint8_t identify_id_qf = get_identity_by_profile_config();
    struct bt_conn *conn;
    struct bt_conn_info info;
    bt_addr_le_t *addr = &profiles[index].peer;
    if (!bt_addr_le_cmp(addr, BT_ADDR_LE_ANY)) {
        return false;
    } else if ((conn = bt_conn_lookup_addr_le(identify_id_qf, addr)) == NULL) {
        return false;
    }
...
#define CHECKED_ADV_STOP()                                                                         \
    ...
    LOG_INF("bt_le_adv_stop ccccccc");                                                             \
    if (err) {                                                                                     \
        LOG_ERR("Failed to stop advertising (err %d)", err);                                       \
        return err;                                                                                \
    }

int zmk_ble_adv_stop(void) {
    if (advertising_status == ZMK_ADV_NONE) {
        LOG_INF("ZMK_ADV_NONE skip stopped");
        return 0;
    }
    LOG_INF("ZMK_ADV_NONE skip stopped 1111");
    int err = bt_le_adv_stop();
    advertising_status = ZMK_ADV_NONE;
    LOG_INF("bt_le_adv_stop dddd");
    if (err) {
        LOG_ERR("Failed to stop advertising (err %d)", err);
        return err;
    }
...
// static uint8_t profile_identity[3] = {
//     1,
//     2,
//     3,
// };
 ...
 * @brief 启动可连接广播
 *
 * @param identify_id 要使用的蓝牙身份ID
 * @return int 0表示成功，负数表示错误
 */

static inline int start_connectable_adv(void) {
    uint8_t identify_id_qf = get_identity_by_profile_config();
    ...
    // LOG_INF("Started connectable advertising with identifyId: %d", identify_id_qf);
    // extern  int start_scanning_qf(void);
    // start_scanning_qf();
    return 0;
}

static inline int start_directed_adv(void) {
    ...
    conn = bt_conn_lookup_addr_le(identify_id_qf, addr);
    if (conn != NULL) {
...
#define CHECKED_DIR_ADV()                                                                          \
    addr = zmk_ble_active_profile_addr();                                                          \
    conn = bt_conn_lookup_addr_le(2, addr);                                                        \
...
int update_advertising(void) {
...
#if IS_ENABLED(CONFIG_SHIELD_CORNE_DONGLE)
    // skip
    return 0;
#else
    // 正常逻辑
    ...
    // LOG_INF("advertising from %d to %d", advertising_status, desired_adv);
    extern int start_scan_qf(void);
    extern void led_set_blink_fast(void);
    ...
    case ZMK_ADV_NONE + CURR_ADV(ZMK_ADV_CONN):
        // LOG_INF("5555 stop adv");
        CHECKED_ADV_STOP();
        led_set_off();
        break;
    case ZMK_ADV_DIR + CURR_ADV(ZMK_ADV_DIR):
    case ZMK_ADV_DIR + CURR_ADV(ZMK_ADV_CONN):
        LOG_INF("2222 stop adv");
        CHECKED_ADV_STOP();
        led_set_off();
        // CHECKED_DIR_ADV();
        start_directed_adv(); // led_set_blink_fast(); 写道里面了
        led_set_blink_fast();
        break;
    case ZMK_ADV_DIR + CURR_ADV(ZMK_ADV_NONE):
        // CHECKED_DIR_ADV();
        LOG_INF("4444  start_directed_adv");
        start_directed_adv();
        led_set_blink_fast();
        break;
    case ZMK_ADV_CONN + CURR_ADV(ZMK_ADV_DIR):
        LOG_INF("5555  CHECKED_ADV_STOP");
        CHECKED_ADV_STOP();
        led_set_off();
        // CHECKED_OPEN_ADV();
        start_connectable_adv();
        led_set_blink_fast();
        break;
    case ZMK_ADV_CONN + CURR_ADV(ZMK_ADV_NONE):
        // CHECKED_OPEN_ADV();
        // LOG_INF("5555  start_connectable_adv");
        start_connectable_adv();

        led_set_blink_fast();
        break;
    default:
        // LOG_INF("11111 nothing get ....");
        // start_scanning_qf();
        break;
    }
...
static void clear_profile_bond(uint8_t profile) {
    if (bt_addr_le_cmp(&profiles[profile].peer, BT_ADDR_LE_ANY)) {
        uint8_t identify_id_qf = get_identity_by_profile_config();
        bt_unpair(identify_id_qf, &profiles[profile].peer);
...
// extern  uint8_t profile_ids[3];
int zmk_ble_prof_select(uint8_t index) {
    if (index >= ZMK_BLE_PROFILE_COUNT) {
        LOG_INF("99999 %d", index);
        return -ERANGE;
    }

    LOG_INF("to switch channel/index %d acive profile %d", index, active_profile);
    ...
    // LOG_INF("22222 %d", index);
    active_profile = index;
    ble_save_profile();

    // LOG_INF("33333 %d", index);
    update_advertising();
    // LOG_INF("44444 %d", index);
...
static void connected(struct bt_conn *conn, uint8_t err) {
    ...
    // LOG_INF("66666 Connected %s", addr);
    extern void led_set_off(void);
#ifdef QF_BLINK_LED
    led_set_off();
#endif
...
extern bool vial_inited;
#include <zephyr/bluetooth/bluetooth.h>
extern void vial_init_work_handler(struct k_work *work);
K_WORK_DELAYABLE_DEFINE(vial_init_work2, vial_init_work_handler);
static int zmk_ble_init(void) {
    ...
    if (!vial_inited) {
        ...
        LOG_INF("Schedule VIAL init");

        k_work_schedule(&vial_init_work2, K_MSEC(500));
    }
